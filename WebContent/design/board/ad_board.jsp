...
<div class="container">
	<div class="row">
		<div class="span12">
			<h1 style="margin-left: 420px; margin-top: 60px; margin-bottom: 60px">간병인을 구합니다</h1>
			<div class="span1"></div>
			<div class="span10">
				...
				<div align="right">
					<form>
						<a href="/khhospital/control?cmd=ADBOARDWRITE" class="btn">글쓰기</a>
					</form>
				</div>
				...
				<div class="pagination pagination-large pagination-centered" >
					<ul>
						<li><c:if test="${nowBlock > 0}">
							<a href="/khhospital/control?cmd=ADBOARDMAIN&nowPage=${pagePerBlock*(nowBlock-1)}&nowBlock=${nowBlock-1}"><<</a>
							</c:if>
						</li>
							<c:forEach begin="1" end="${pagePerBlock}" step="1" var="i">
							<c:if test="${(nowBlock * pagePerBlock) +i-1 < totalPage}">
								<li><a href="/khhospital/control?cmd=ADBOARDMAIN&nowPage=${(nowBlock*pagePerBlock)+i-1}&nowBlock=${nowBlock}">${(nowBlock * pagePerBlock)+i}</a></li>
							</c:if>
							</c:forEach>
						<li>
				...
				<div align="center" style="margin-bottom: 100px">
					<form class="form-search" method="post" action="/khhospital/control" name="search">		
						<input type="hidden" name="cmd" value="ADBOARDMAIN"/>
						<input type="hidden" name="num" />					
						<select class="span1" name="keyField" id="keyField">
							<option value="adb_title_main" >제목</option>
							<option value="adb_name_main" >작성자</option>
							<option value="adb_content_main" >내용</option>
							<option value="adb_titlecontent_main" >제목+내용</option>
						</select>
