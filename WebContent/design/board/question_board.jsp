...
<div class="container">
	<div class="row">
		<h1 style="margin-left: 500px; margin-top: 60px; margin-bottom: 60px">문의하기</h1>
		...
		<div class="span10">
			...
			<div align="right">			
				<a href="/khhospital/control?cmd=QBOARDWRITE" class="btn">글쓰기</a>			
			</div>
			<div class="pagination pagination-large pagination-centered" >
				<ul>
					<li><c:if test="${nowBlock > 0}">
						<a href="/khhospital/control?cmd=QBOARDMAIN&nowPage=${pagePerBlock*(nowBlock-1)}&nowBlock=${nowBlock-1}"><<</a>
						</c:if>
					</li>
						<c:forEach begin="1" end="${pagePerBlock}" step="1" var="i">
						<c:if test="${(nowBlock * pagePerBlock) +i-1 < totalPage}">
							<li><a href="/khhospital/control?cmd=QBOARDMAIN&nowPage=${(nowBlock*pagePerBlock)+i-1}&nowBlock=${nowBlock}">${(nowBlock * pagePerBlock)+i}</a></li>
						</c:if>
						</c:forEach>
					<li>
			...
			<div align="center" style="margin-bottom: 100px">
				<form class="form-search" method="post" action="/khhospital/control" name="search">		
					<input type="hidden" name="cmd" value="QBOARDMAIN"/>
					<input type="hidden" name="num" />
					<select class="span1" name="keyField" id="keyField" >
						<option value="qb_title_main" >제목</option>
						<option value="qb_name_main" >작성자</option>
						<option value="qb_content_main" >내용</option>
						<option value="qb_titlecontent_main" >제목+내용</option>
					</select>
...
<form name="read" method="POST" action="/khhospital/control">
	<input type="hidden" name="cmd" value="QBOARDREAD"/>
	<input type="hidden" name="num"/>
