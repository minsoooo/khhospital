...
<script>
		$(document).ready(
			function(){
				$("#board").css("margin-left","180px");
				$("#boardName").css("text-align","center").css("font-size","30px")
				.css("color","#595959").css("margin-bottom","20px")
			}	
		);
</script>
...
<div class="container">
	<div class="row">
		<div class="span12">
			<h1 style="margin-left: 450px; margin-top: 60px; margin-bottom: 20px">관리자 공지사항</h1>
			<div class="span1"></div>
			<div class="span10" style="margin-bottom: 60px">
				...
				<div align="right">
					<a href="/khhospital/control?cmd=MGRNOTICEBOARDWRITE" class="btn">글쓰기</a>			
				</div>
				<div class="pagination pagination-large pagination-centered" >
					<ul>
						<li><c:if test="${nowBlock > 0}">
							<a href="/khhospital/control?cmd=MGRNOTICEBOARDMAIN&nowPage=${pagePerBlock*(nowBlock-1)}&nowBlock=${nowBlock-1}"><<</a>
							</c:if>
						</li>
							<c:forEach begin="1" end="${pagePerBlock}" step="1" var="i">
							<c:if test="${(nowBlock * pagePerBlock) +i-1 < totalPage}">
								<li><a href="/khhospital/control?cmd=MGRNOTICEBOARDMAIN&nowPage=${(nowBlock*pagePerBlock)+i-1}&nowBlock=${nowBlock}">${(nowBlock * pagePerBlock)+i}</a></li>
							</c:if>
							</c:forEach>
						<li>
				...
				<div align="center">
					<form class="form-search" method="post" action="/khhospital/control" name="search">		
						<input type="hidden" name="cmd" value="MGRNOTICEBOARDMAIN"/>
						<input type="hidden" name="num" />
						<select class="span1" name="keyField" id="keyField" >
							<option value="nb_title_main" >제목</option>
							<option value="nb_name_main" >작성자</option>
							<option value="nb_content_main" >내용</option>
							<option value="nb_titlecontent_main" >제목+내용</option>
						</select>
...
<form name="read" method="POST" action="/khhospital/control">
	<input type="hidden" name="cmd" value="MGRNOTICEBOARDREAD"/>
	<input type="hidden" name="num"/>
	<input type="hidden" name="isRead" value="true"/>
	<input type="hidden" name="mgr" value="true"/>
</form>	
