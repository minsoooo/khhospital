...
<div class="container">
	<div class="row">
		<h1 style="margin-left: 500px; margin-top: 60px; margin-bottom: 20px">관련뉴스</h1>
		...
		<div class="span10">
			...
			<div align="right">
					<a href="/khhospital/control?cmd=MGRMEDICALNEWSBOARDWRITE" class="btn">글쓰기</a>			
				</div>
			<div class="pagination pagination-large pagination-centered" >
				<ul>
					<li><c:if test="${nowBlock > 0}">
						<a href="/khhospital/control?cmd=MGRMEDICALNEWSBOARDMAIN&nowPage=${pagePerBlock*(nowBlock-1)}&nowBlock=${nowBlock-1}"><<</a>
						</c:if>
					</li>
						<c:forEach begin="1" end="${pagePerBlock}" step="1" var="i">
						<c:if test="${(nowBlock * pagePerBlock) +i-1 < totalPage}">
							<li><a href="/khhospital/control?cmd=MGRMEDICALNEWSBOARDMAIN&nowPage=${(nowBlock*pagePerBlock)+i-1}&nowBlock=${nowBlock}">${(nowBlock * pagePerBlock)+i}</a></li>
						</c:if>
						</c:forEach>
					<li>
			...
			<div align="center">
				<form class="form-search" method="post" action="/khhospital/control" name="search">		
					<input type="hidden" name="cmd" value="MGRMEDICALNEWSBOARDMAIN"/>
					<input type="hidden" name="num" />
					<select class="span1" name="keyField" id="keyField" >
						<option value="newsb_title_medical" >제목</option>
						<option value="newsb_name_medical" >작성자</option>
						<option value="newsb_content_medical" >내용</option>
						<option value="newsb_titlecontent_medical" >제목+내용</option>
					</select>
...
<form name="read" method="POST" action="/khhospital/control">
	<input type="hidden" name="cmd" value="MGRMEDICALNEWSBOARDREAD"/>
	<input type="hidden" name="num"/>
