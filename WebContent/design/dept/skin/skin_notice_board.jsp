...
<c:set var="beginPerPage" value="${pdto.beginPerPage}"></c:set>
	...
	<div class="container">
		<div class="row">
			<div class="span12">
				<h1 style="margin-left: 450px; margin-top: 60px; margin-bottom: 20px">피부과 공지사항</h1>
				<div class="span1"></div>
				<div class="span10" style="margin-bottom: 60px">
					...
					<div class="pagination pagination-large pagination-centered" >
						<ul>
							<li><c:if test="${nowBlock > 0}">
								<a href="/khhospital/control?cmd=SKINNBOARDMAIN&nowPage=${pagePerBlock*(nowBlock-1)}&nowBlock=${nowBlock-1}"><<</a>
								</c:if>
							</li>
								<c:forEach begin="1" end="${pagePerBlock}" step="1" var="i">
								<c:if test="${(nowBlock * pagePerBlock) +i-1 < totalPage}">
									<li><a href="/khhospital/control?cmd=SKINNBOARDMAIN&nowPage=${(nowBlock*pagePerBlock)+i-1}&nowBlock=${nowBlock}">${(nowBlock * pagePerBlock)+i}</a></li>
								</c:if>
								</c:forEach>
							<li>
					...
					<div align="center">
						<form class="form-search" method="post" action="/khhospital/control" name="search">		
							<input type="hidden" name="cmd" value="SKINNBOARDMAIN"/>
							<input type="hidden" name="num" />
							<select class="span1" name="keyField" id="keyField" >
								<option value="nb_title_skin" >제목</option>
								<option value="nb_name_skin" >작성자</option>
								<option value="nb_content_skin" >내용</option>
								<option value="nb_titlecontent_skin" >제목+내용</option>
							</select>
...
<form name="read" method="POST" action="/khhospital/control">
	<input type="hidden" name="cmd" value="SKINNBOARDREAD"/>
	<input type="hidden" name="num"/>
