		...
		<div class="row">
			<div class="span12">
				...
				<div class="span10" style="margin-bottom: 60px">
					...
					<div align="right">
						<a href="/khhospital/control?cmd=DOCDENTQBOARDWRITE" class="btn">글쓰기</a>			
					</div>
					<div class="pagination pagination-large pagination-centered" >
						<ul>
							<li><c:if test="${nowBlock > 0}">
								<a href="/khhospital/control?cmd=DOCDENTQBOARDMAIN&nowPage=${pagePerBlock*(nowBlock-1)}&nowBlock=${nowBlock-1}"><<</a>
								</c:if>
							</li>
								<c:forEach begin="1" end="${pagePerBlock}" step="1" var="i">
								<c:if test="${(nowBlock * pagePerBlock) +i-1 < totalPage}">
									<li><a href="/khhospital/control?cmd=DOCDENTQBOARDMAIN&nowPage=${(nowBlock*pagePerBlock)+i-1}&nowBlock=${nowBlock}">${(nowBlock * pagePerBlock)+i}</a></li>
								</c:if>
								</c:forEach>
							<li>
					...
					<div align="center">
						<form class="form-search" method="post" action="/khhospital/control" name="search">		
							<input type="hidden" name="cmd" value="DOCDENTQBOARDMAIN"/>
							<input type="hidden" name="num" />
							<select class="span1" name="keyField" id="keyField" >
								<option value="qb_title_dent" >제목</option>
								<option value="qb_name_dent" >작성자</option>
								<option value="qb_content_dent" >내용</option>
								<option value="qb_titlecontent_dent" >제목+내용</option>
							</select>
...
<form name="read" method="POST" action="/khhospital/control">
	<input type="hidden" name="cmd" value="DOCDENTQBOARDREAD"/>
	<input type="hidden" name="num"/>
